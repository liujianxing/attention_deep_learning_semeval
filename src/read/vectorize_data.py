...
def vectorize_rnn(train_sentences, dev_sentences, test_sentences, c=None):
    ...
    model = w2v.load_vectors(c.W2VEC_FILE)
    ...
    x_train, y_train, t_train, a_train, p_train, s_train = sentences_to_x_y_l(train_arr, ind, max, aspects, c)
    x_dev, y_dev, t_dev, a_dev, p_dev, s_dev = sentences_to_x_y_l(dev_arr, ind, max, aspects, c)
    x_test, y_test, t_test, a_test, p_test, s_test = sentences_to_x_y_l(test_arr, ind, max, aspects, c)
...
def sentences_to_x_y_l(all_sentences, ind, max, aspects, c):
    ...
    return X, y, l, a, s, sentences

def sentences_to_target_expressions(all_sentences, ind, max, aspects, c):
    return None
